...
function realmaximize(win)
  local screen = win:screen_of()
  
  if not screen then return end

  -- Capture the currently focused region
  --local prev = notioncore.current()

  local geom = screen:geom()
  win:rqgeom({x = 0, y = 0, w = geom.w, h = geom.h})
  WRegion.rqorder(win, 'front')

  WRegion.goto_focus(win)
  -- Restore focus to the previously focused region
  --if prev then
  --  region_goto(prev)
  --end
end
